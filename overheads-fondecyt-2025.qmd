...
overheads <- data |>
  ...
  ppto_2025_solicitado= ppto_2025/1.2,
  overheads = ppto_2025_solicitado*0.2,
...
overheads |>
  rename(
    ...
    `Ppto anual total ($)` = ppto_2025,
    "Ppto solicitado 2025 ($)" = ppto_2025_solicitado,
    `Overheads Total ($)` = overheads,
    `Overheads DIP ($)` = overheads_dip
  ) |>
  gt() |>
  fmt_number(
    columns = c(`Ppto anual total ($)`, "Ppto solicitado 2025 ($)", `Overheads Total ($)`, `Overheads DIP ($)`),
    decimals = 0, sep_mark = ".", dec_mark = ","   # separador de miles
  ) |>
  grand_summary_rows(
    columns = c(`Ppto anual total ($)`, `Overheads Total ($)`, "Ppto solicitado 2025 ($)", `Overheads DIP ($)`),
